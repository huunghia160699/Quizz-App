document.addEventListener("DOMContentLoaded", () => {
  ...
  const quizApp = {
        ...
        questionBlocks.forEach((block) => {
            ...
            lines.forEach(line => {
                if (line.match(/^Câu\s*\d+[:.]?\s*/i)) {
                    ...
                    const answerLetter = line.substring(line.indexOf(":") + 1).trim().toUpperCase();
            ...
            if (question.text && Object.keys(question.options).length > 0 && question.correctAnswer) {
            ...
            mammoth.extractRawText({ arrayBuffer: arrayBuffer })
                .then(result => {
                    ...
                    if (this.originalQuestions.length > 0) {
                        ...
                        alert("Không tìm thấy dữ liệu câu hỏi hợp lệ trong file. Vui lòng kiểm tra lại định dạng file Word.");
                        this.resetApp();
                    }
                })
                .catch(err => {
                    console.error(err);
                    alert("Đã có lỗi xảy ra khi xử lý file Word. File có thể bị lỗi hoặc không đúng định dạng.");
                    this.resetApp();
                });
        };
    ...
    loadQuestion() {
        ...
        this.ui.questionCounterDisplay.textContent = `Câu ${this.currentQuestionIndex + 1} / ${this.questionsToPlay}`;
    ...
    showResult() {
        ...
        this.ui.restartIncorrectButton.disabled = this.incorrectlyAnswered.length === 0;
    },
    ...
    bindEvents() {
        this.ui.fileInput.addEventListener("change", this.handleFileSelect.bind(this));
        ...
        this.ui.startButton.onclick = () => {
            ...
            const questionsForThisRound = this.shuffleArray([...this.originalQuestions]).slice(0, num);
